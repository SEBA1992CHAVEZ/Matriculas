...
class Apoderado {
    ...
    static async getById(id) {
        const [rows] = await db.execute('SELECT * FROM apoderados WHERE id_apoderado = ?', [id]);
        return rows[0];
    }
    ...
    static async create(data) {
        ...
        const sql = `INSERT INTO apoderados (rut_apoderado, nombres, apellidos, telefono) VALUES (?, ?, ?, ?)`;
        const [result] = await db.execute(sql, [rut_apoderado, nombres, apellidos || null, telefono || null]);
    ...
    static async update(id, data) {
        ...
        const sql = `UPDATE apoderados SET nombres = ?, apellidos = ?, telefono = ? WHERE id_apoderado = ?`;
        await db.execute(sql, [nombres, apellidos || null, telefono || null, id]);
    }
    ...
    static async delete(id) {
        await db.execute('DELETE FROM apoderados WHERE id_apoderado = ?', [id]);
    }
    ...
    static async associateToAlumno(id_alumno, id_apoderado, id_parentesco, es_titular) {
        const sql = `INSERT INTO detalle_apoderados_alumno (id_alumno, id_apoderado, id_parentesco, es_titular) VALUES (?, ?, ?, ?)`;
        await db.execute(sql, [id_alumno, id_apoderado, id_parentesco, es_titular]);
    }
}
